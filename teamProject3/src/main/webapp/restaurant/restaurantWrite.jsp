	...
	<form name="form" id="form" method="post" align="center"
		...
		<div>
			...
			<input type="text" id="sample4_roadAddress" name="res_si"
				...
				id="sample4_detailAddress" placeholder="상세주소" hidden> <input
				type="text" id="sample4_extraAddress" placeholder="참고항목" hidden>

		</div>
		<br>
		<div>
			영업시간 <select name=startH>
				<c:forEach begin="0" end="24" var="i">
					<option value="${i}">${i}</option>
				</c:forEach>
			</select> : <select name=startM>
				<c:forEach begin="0" end="60" var="i">
					<option value="${i}">${i}</option>
				</c:forEach>
			</select> ~ <select name=endH>
				<c:forEach begin="0" end="24" var="i">
					<option value="${i}">${i}</option>
				</c:forEach>
			</select> : <select name=endM>
				<c:forEach begin="0" end="60" var="i">
					<option value="${i}">${i}</option>
				</c:forEach>
			</select>
		...
		<div>
			<input type="file" id="input_imgs" name="res_picture" accept=".jpg,.jpeg,.png,.gif,.bmp" multiple />
		</div>
		<br>
		<div>
			<div class="imgs_wrap">
